...
<?php
    ...
    if(isset($_POST['update'])) {
        try {
			$sql = "update staff
                    set staffImage = ?, staffName = ?, staffPhone = ?, 
                        staffEmail = ?, staffRole = ?
                    where staffID = ?";

            $image = $_POST['staffImage'] == '' ? $_POST['old_image'] : $_POST['staffImage'];        
			$stmt = $conn->prepare($sql);
			$stmt->bindParam(1, $image);	
			$stmt->bindParam(2, $_POST['name']);
			$stmt->bindParam(3, $_POST['phone']);
			$stmt->bindParam(4, $_POST['email']);
			$stmt->bindParam(5, $_POST['role']);   
                                    ...
                                    <form class="forms-sample" method="POST"
                                        ...
                                        <div class="form-group">
                                            ...
                                            <input type="text" class="form-control" id="staffID" readonly
                                                value="<?= $row_edit['staffID']?>" name="id">
                                        </div>
                                        <div class="form-group">
                                            <label>Image:</label>
                                            <img src="../../image/staff/<?= $row_edit['staffImage'] ?>" height="80px"
                                                width="70px" alt="no image">
                                            <input type="hidden" name="old_image" value="<?=$row_edit['staffImage'] ?>">
                                            <input type="file" name="staffImage" class="file-upload-default">
                                        ...
                                        <div class="form-group">
                                            ...
                                            <input type="text" class="form-control" id="staffName"
                                                placeholder="Enter staff name" value="<?= $row_edit['staffName']?>"
                                                name="name">
                                        </div>
                                        <div class="form-group">
                                            <label for="staffPhone">Phone: </label>
                                            <input type="text" class="form-control" id="staffPhone"
                                                placeholder="Enter staff phone" value="<?= $row_edit['staffPhone']?>"
                                                name="phone">
                                        </div>
                                        <div class="form-group">
                                            <label for="staffEmail">Email: </label>
                                            <input type="text" class="form-control" id="staffEmail"
                                                placeholder="Enter staff phone" value="<?= $row_edit['staffEmail']?>"
                                                name="email">
                                        </div>
                                        <div class="form-group">
                                            <label for="staffRole">Email: </label>
                                            <input type="text" class="form-control" id="staffRole"
                                                placeholder="Enter staff phone" value="<?= $row_edit['staffRole']?>"
                                                name="role">
                                        </div>
                                        ...
                                        <a href="staff.php" class="btn btn-light">Back</a>
                                    </form>
                                </div>
                            </div>
                        </div>
